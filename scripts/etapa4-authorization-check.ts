...
assert(isTenantRole("CLIENT"), "tenant role");
assert(isInternalRole("ADMIN") && isInternalRole("OPERATOR"), "internal roles");
assert(!isInternalRole("CLIENT") && !isTenantRole("ADMIN"), "domains must not overlap");
assert(!isTenantRole("OPERATOR"), "OPERATOR is not tenant");

assert(hasAllowedRole(admin, ["ADMIN"]), "admin allowlist");
assert(!hasAllowedRole(ownerA, ["ADMIN"]), "OWNER is not Cartori ADMIN");
assert(!hasAllowedRole(operator, ["ADMIN"]), "OPERATOR is not ADMIN");
assert(!hasAllowedRole(clientA, ["ADMIN"]), "CLIENT denied for admin allowlist");
...
assert(canAccessOrganization(orgAdminA, "org-a"), "ADMIN ACTIVE own org");
...
assert(!canOnboardBusiness(memberA), "MEMBER cannot onboard");
